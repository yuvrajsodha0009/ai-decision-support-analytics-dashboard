...
const buildDenseChartSeries = ({
  ...
  const parsedRows = rows
    .map((row) => {
      ...
      return {
        ...
        previousValue: compareMode ? toChartNumber(row[metric.previousKey]) : null,
      };
    })
    .filter(Boolean)
  ...
  const mapByBucket = new Map(
    parsedRows.map((row) => [getBucketKey(new Date(row.period), groupBy, timeZone), row])
  );
...
const TrendTooltip = ({ active, payload, label, metric, compareMode }) => {
  ...
  return (
    <div className="rounded-xl border border-slate-500/35 bg-slate-900/85 px-4 py-3 shadow-xl backdrop-blur-md">
      <p className="mb-2 text-sm font-semibold text-slate-100">{formatDateLabel(label)}</p>
      {currentValue !== null && (
        <p className="text-sm text-cyan-200">
          Current {metric.label}: <span className="font-semibold">{metric.formatValue(currentValue)}</span>
        </p>
      )}
      {compareMode && previousValue !== null && (
        <p className="mt-1 text-sm text-orange-200">
          Previous {metric.label}:{" "}
          <span className="font-semibold">{metric.formatValue(previousValue)}</span>
        </p>
      )}
    </div>
  );
...
const RevenueTrendChart = ({ data, compareMode, loading, error }) => {
  ...
  const actions = (
    <>
      ...
      >
        {compareMode ? "Comparing Previous Period" : "Compare with Previous Period"}
      </button>
    </>
  );
  ...
  return (
    <ChartContainer
      ...
      subtitle="Switch between revenue, orders, and AOV with optional previous-period comparison"
      actions={actions}
      loading={loading}
      error={error}
      ...
      <div className="h-full w-full overflow-x-auto pb-1">
        <div className="h-full min-w-[760px]">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={chartData} margin={{ top: 10, right: 18, left: 0, bottom: 10 }}>
              <defs>
                <linearGradient id={areaGradientId} x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor={metric.areaGradient[0]} stopOpacity={0.35} />
                  <stop offset="95%" stopColor={metric.areaGradient[1]} stopOpacity={0} />
                </linearGradient>
              </defs>

              <CartesianGrid vertical={false} stroke="#475569" strokeOpacity={0.16} />
              <XAxis
                dataKey="period"
              ...
              <Tooltip
                ...
                content={<TrendTooltip metric={metric} compareMode={compareMode} />}
              />
